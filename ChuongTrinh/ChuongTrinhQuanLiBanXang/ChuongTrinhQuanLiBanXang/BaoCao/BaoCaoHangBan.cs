...
{
    public partial class BaoCaoHangBan : DevExpress.XtraReports.UI.XtraReport
    {
        int Nam;
        public BaoCaoHangBan(int year)
        {
            Nam = year;
            InitializeComponent();
        ...
        Color[] color =  { Color.LightGreen, Color.Red};
        private void TaoFormHangBan()
        {
            QuanLiBanXangDataContext DB = new QuanLiBanXangDataContext();
            float i = 0;
            float vtThang = 0;
            int nam=Nam;
            for (int thang = HangBanDLL.LayRaThangNhoNhat(nam); thang <= HangBanDLL.LayRaThangLonNhat(nam); thang++)
            {
                ...
                lableDienGiai.HeightF = HangBanDLL.LoadALL(thang,nam).Count * 20f;
                lableDienGiai.TextAlignment = DevExpress.XtraPrinting.TextAlignment.MiddleCenter;
                lableDienGiai.Text = "Tháng " + thang;
                vtThang = vtThang + HangBanDLL.LoadALL(thang,nam).Count * 20f;
                foreach(BaoCaoTong Ds in tongCongBaoCaoBLL.LoadHangBan(thang,nam))
                {
                    ...
                    lableTenTongThanhTien.HeightF = 20f;
                    ...
                    {
                        lableKhac.Text = ((int)Ds.SoLuong).ToString();
                ...
                foreach (HangBan Ds in HangBanDLL.LoadALL(thang, nam))
                {
                    ...
                    lable.Text = Ds.NgayBan.Value.Day + "-" + Ds.NgayBan.Value.Month;
                ...
                i += (float)MatHangBLL.LoadMatHang().Count * 20+20f;
